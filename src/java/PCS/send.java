...
public class send extends HttpServlet
{
    ...
    throws ServletException, IOException
    {
        ...
        String des = request.getParameter("details");
        String cate = request.getParameter("state");
        String details = request.getParameter("details");
        String uid = (String)sessionobj.getAttribute("eid");
        String docid = request.getParameter("docid");
        String projid =(String)sessionobj.getAttribute("project");
        String pro =(String)sessionobj.getAttribute("pro");
       ...
       try
        {
              ...
              while(rs.next())
              {
              rd = sd.executeQuery("SELECT CURRENT_TIME()");
              while(rd.next())
              {
              ...
              if(uid.equals("Admin"))
              {
                     st.executeUpdate("INSERT INTO Document_Master (Document_ID,Sent_From,Sent_To,Document_Status,Project_ID,Description,Date) VALUES('"+docid+"','"+uid+"','"+sendto+"','"+cate+"','"+pro+"','"+details+"','"+d1+"' )");
